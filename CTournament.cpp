...
namespace Tournament
{
    ...
    void Tournament1v1DE(vector <CCharacter> J, unsigned NbJoueur, vector <vector <string>> MPerso)
    {
    	...
    	unsigned NbJoueurR1, NbJoueurR0;
		...
		while (MatchesR0.size() < NbJoueurR0Eff / 2)
		{
			...
			unsigned RandomNum;
			RandomNum = rand() % (Participant.size()); //nombre entre 0 et 3 // ?? (19/9/2017)
		...
		//Déroulement des matchs Round 0 Winner avec choix de match
			...
			for (unsigned j (0); j < J.size(); ++j)
			{
				if (JGagnant == MatchesR0[i].first.GetPlayer())
					if ((J[j].GetPlayer() == JGagnant && J[j].GetNumPerso() == MatchesR0[i].first.GetNumPerso()))
					{
						...
						Perdant.push_back(MatchesR0[i].second);
						break;
					}
				if (JGagnant == MatchesR0[i].second.GetPlayer())
					...
					{
						Gagnant.push_back(J[j]); //augmentation du nombre de victoire
						Perdant.push_back(MatchesR0[i].first);
						break;
					}
			}
			system("cls");
		...
		{
			pair <CCharacter, CCharacter> Match;
			Match.first = Perdant[i];
			Match.second = Perdant[i+1];
			Matches.push_back(Match);
		}

		Perdant.clear();

		//Déroulement des matchs Round 0 Loser avec choix de match
			...
			if (Choix[i])
			{
				...
				for (unsigned j (0); j < J.size(); ++j)
				{
					if (PGagnant == Matches[i].first.GetNumPerso())
						...
						{
							Perdant.push_back(J[j]); //augmentation du nombre de victoire
							break;
						}
		...
		//Déroulement des matchs Round 1 Winner avec choix de match
			...
			if (Choix[i])
			{
				...
				for (unsigned j (0); j < J.size(); ++j)
				{
					if (PGagnant == Matches[i].first.GetNumPerso())
						...
						{
							Gagnant.push_back(J[j]); //augmentation du nombre de victoire
							Perdant.push_back(Matches[i].second);
							break;
						}
					if (PGagnant == Matches[i].second.GetNumPerso())
						...
						{
							Gagnant.push_back(J[j]); //augmentation du nombre de victoire
							Perdant.push_back(Matches[i].first);
							break;
						}
				}
			}
		...
		//Déroulement des matchs Round 1 Loser avec choix de match
		...
		while (Gagnant.size() > 2)
		{
			...
			cout << "Round " << Round << " Winner Side : " << endl << endl;
				...
				else
				{
					...
					for (unsigned j (0); j < J.size(); ++j)
					{
						...
						if (JGagnant == Matches[i].second.GetPlayer())
							...
							{
								Gagnant.push_back(J[j]); //augmentation du nombre de victoire
								Perdant.push-back(Matches[i].first);
								break;
							}
					}
				}
